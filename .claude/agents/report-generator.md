...
2-3 sentence non-technical summary of what was accomplished.

## Business Value

How this advances the PoC goals (bike authentication at 500K scale).

## Risk Assessment

Any risks introduced or mitigated by this change.
```
...
How this PR advances its step. Updated completion percentage.

## Overall Progress

| Step | Name | Status | Progress | Merged PRs | Open PRs |
|------|------|--------|----------|------------|----------|
| 1 | Choose Authentication Approach | Complete | 100% | #10, #11, #12 | -- |
| 2 | Infrastructure Setup | In Progress | NN% | #3, #8, #9 | #14-#19, #25 |
| ... | ... | ... | ... | ... | ... |

## Blockers

- Blocker 1 (if any identified)
```
...
- **Level 2**: UPDATE `cumulative/level2-executive-summary.md` (rewrite Recent Accomplishments section, keep history)
- **Level 3**: RECALCULATE `cumulative/level3-project-tracker.md` (rebuild table with new completion percentages)
...
Split "Key Accomplishments" into three subsections:

```markdown
### Shared Foundation
(accomplishments from shared PRs)

### Spectrum Path (Active)
(accomplishments from spectrum PRs + cross-repo PRs)

### NLB Path (Pending Contract)
(accomplishments from nlb PRs)
```

### Level 3 — Cumulative PM Tracker

Add a "Path" column to the step progress table. For Step 7, render as two sub-rows:

```markdown
| 7a | Edge Layer — Spectrum | In Progress | NN% | #NN | #NN |
| 7b | Edge Layer — NLB | In Progress | NN% | #21, #22 | #41 |
```

Add a `## Path Progress Summary` table after the main step table:

```markdown
## Path Progress Summary

| Path | Status | Merged PRs | Open PRs | Cross-Repo PRs |
|------|--------|------------|----------|----------------|
| Spectrum | Active | N | M | K |
| NLB | Pending contract | N | M | -- |
| Shared | -- | N | M | -- |
```
...
- **Current Phase**: Must state the actual step completion percentage, list remaining open PRs by number, and clearly state whether infrastructure has been provisioned or is only code
- **Business Value**: Only claim value for what's actually accomplished (code reviewed, designs complete), not for running infrastructure unless confirmed
- **Risk Assessment**: If a step is partially complete, the remaining open PRs AND unprovisioned infrastructure ARE risks -- state them explicitly
